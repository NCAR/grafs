...
def main():
    ...
    plt.grid()
    plt.title("Valid Observation Percentage")
    plt.colorbar()
    plt.show()
    return
...
class ObsQualityControl(object):
    ...
    def clearness_index(self):
        new_columns = ["zenith", "elevation", "azimuth", "apparent_zenith", "etr", "clearness_index",
        ...
        for station in self.station_data.index:
            ...
            sol_pos["etr"] = extraradiation(station_dates)
            sol_pos["clearness_index"] = self.data.loc[station_ix, self.variable].values / \
